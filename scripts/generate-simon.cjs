...
const imgPath = path.join(__dirname, '../artifacts/os-history/public/simon67.jpg');
const b64 = fs.readFileSync(imgPath).toString('base64');

const tsCode = `import { Router, type IRouter } from "express";

const router: IRouter = Router();

const SIMON_IMAGE_B64 = "${b64}";

router.get("/simon67", (_req, res) => {
  const imgSrc = "data:image/jpeg;base64," + SIMON_IMAGE_B64;
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .frame-wrap:hover { transform: scale(1.03); }
    .neon-frame {
      padding: 8px;
...
<body>
  ...
  <div class="facts">
    ...
    <div class="fact"><span>🏆</span> <span>Puntaje ICFES: <strong>472</strong> puntos — nivel máximo</span></div>
    <div class="fact"><span>🚀</span> <span>Orgullo de Neiva, Huila</span></div>
    <div class="fact"><span>🐧</span> <span>Servidor API Server — Activo</span></div>
  </div>

  <a href="https://sistema-operativo-six.vercel.app/simon67" class="btn">← Ir al Museo de Sistemas Operativos</a>
</body>
</html>\`);
});
...
const targetPath = path.join(__dirname, '../artifacts/api-server/src/routes/simon.ts');
fs.writeFileSync(targetPath, tsCode);
console.log('Successfully written simon.ts with embedded base64!');
